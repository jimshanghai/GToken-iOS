...
class BindViewModel: NSObject {
    weak var target:BindViewController!
        //注册
        func regitser(dic:NSDictionary){
            SVProgressHUD.show(withStatus: NSLocalizedString("正在加载", comment: ""))
            NetworkTools.shareInstance.request(methodType: .POST, urlString: API_keystoreregist, params: dic as? [String : AnyObject], success: { (obj) in
                ...
                if(result.object(forKey: "code")as? Int == 200 ){
                    ...
                    if(!((result.object(forKey: "data") as!NSDictionary)["mobile"] is NSNull)){
                    ...
                    UserDefaults.standard.set((result.object(forKey: "data")as!NSDictionary)["keystore"], forKey: "keystorejson")
                     UserDefaults.standard.set(true, forKey: "needInfoAlert")
