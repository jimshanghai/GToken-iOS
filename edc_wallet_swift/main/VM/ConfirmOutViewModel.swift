...
class ConfirmOutViewModel: NSObject {
    weak var target:ConfirmViewController!
    
    func nextActionSignal(dic:NSDictionary) {
        ...
        NetworkTools.shareInstance.request(methodType: .POST, urlString: API_totransfer, params: (dic as! [String : AnyObject]) , success: { (obj) in
            ...
            if(result.object(forKey: "code")as?Int == 200 ){
                self.target.navigationController?.pushViewController(SuccessViewController(), animated: true)
            }
            else {
